// packages/engine/src/errors/codes.ts
//
// Teljes hibakód-regiszter — a Soborbo flotta KÖZÖS katalógusa, az
// `error-pipeline` repo `src/codes.ts`-éből átvéve (egy az egyben, hogy minden
// shop ugyanazokat a kódokat használja). A PIPELINE-t NEM vesszük át: itt nincs
// `console.error` → Tail Worker → KV transport. A hibák kizárólag a belső
// `error_log` Postgres-táblába íródnak (lásd `logger.ts` + `db/schema.ts`).
//
// SECURITY: ebben a modulban SOHA nincs `console.*`. Cloudflare Workeren minden
// console hívás a megfigyelési / Tail-stream-be kerül, ami a bizalmi határon
// kívülre szivárogtathat. A shadow-ellenőrzés ezért console helyett egy
// exportált tömbbe gyűjt (a teszt assertálja, hogy üres).
//
// Minden kód FIXÁLT severity + retryable + userImpact.
// requiredContext: ezek a context mezők KÖTELEZŐEK ehhez a kódhoz; ha
// nélkülük naplózol, a logger a hiányzókat a context `_missingContext`
// mezőjébe írja (szintén nem console-ra).
//
// Saját, engine-specifikus kódokat a PROJECT_CODES blokkba vegyél fel.
...
// Engine-specifikus kódok (Soborbo webshop core). Prefixek — DB, CART, ORDER,
// PAY, INV, SHIP, SEARCH, AUTH — egyik sem ütközik a globális katalógussal.
// `satisfies` (nem annotáció), hogy a kulcsok literálok maradjanak és
// `EngineErrorCode` egy pontos union legyen → engine kód csak létező kódot
// naplózhat (típushiba különben). Ez adja a "mindig tudjuk mi történik"-et.
export const PROJECT_CODES = {
  // -- Adatbázis (Hyperdrive → Supabase direct connection, ADR-0004) --------
  "DB-CONN-001":        { severity: "CRITICAL", retryable: false, userImpact: "blocked",  message: "Nincs DB connection string (HYPERDRIVE / DATABASE_URL hiányzik)", requiredContext: ["source"] },
  "DB-QUERY-001":       { severity: "ERROR",    retryable: true,  userImpact: "degraded", message: "Drizzle query dobott", requiredContext: ["source", "errorMessage"] },
  "DB-TX-001":          { severity: "ERROR",    retryable: true,  userImpact: "blocked",  message: "Tranzakció rollback (több táblát érintő írás)", requiredContext: ["source", "errorMessage"] },

  // -- Kosár (server-side price authority) ----------------------------------
  "CART-VARIANT-001":   { severity: "WARN",     retryable: false, userImpact: "degraded", message: "Kosár variant hiányzik vagy archivált", requiredContext: ["variantId"] },
  "CART-PRICE-001":     { severity: "ERROR",    retryable: false, userImpact: "blocked",  message: "Nincs ár a variantra ebben a pénznemben", requiredContext: ["variantId", "currency"] },
  "CART-STOCK-001":     { severity: "INFO",     retryable: false, userImpact: "degraded", message: "Kosárba tett tétel készlethiányos", requiredContext: ["variantId"] },

  // -- Pénztár / rendelés (server-computed totals authority) ----------------
  "ORDER-CART-EMPTY-001": { severity: "INFO",   retryable: false, userImpact: "blocked",  message: "Checkout üres kosárral", requiredContext: ["source"] },
  "ORDER-CREATE-001":   { severity: "ERROR",    retryable: false, userImpact: "blocked",  message: "Rendelés létrehozás sikertelen", requiredContext: ["source", "errorMessage"] },
  "ORDER-PRICE-001":    { severity: "CRITICAL", retryable: false, userImpact: "blocked",  message: "Szerveroldali ár-újraszámítás eltérés", requiredContext: ["orderId", "clientTotal", "serverTotal"] },
  "ORDER-VANISHED-001": { severity: "CRITICAL", retryable: false, userImpact: "blocked",  message: "Rendelés eltűnt közvetlenül létrehozás után", requiredContext: ["orderId"] },

  // -- Fizetés (PaymentProvider absztrakció: Viva / Stripe, ADR-0005) -------
  "PAY-CONFIG-001":     { severity: "CRITICAL", retryable: false, userImpact: "blocked",  message: "Payment provider secret nincs konfigurálva", requiredContext: ["provider"] },
  "PAY-CURRENCY-001":   { severity: "ERROR",    retryable: false, userImpact: "blocked",  message: "Nincs payment provider ehhez a pénznemhez", requiredContext: ["currency"] },
  "PAY-INIT-001":       { severity: "ERROR",    retryable: true,  userImpact: "blocked",  message: "Payment initiate (hosted checkout) sikertelen", requiredContext: ["provider", "orderId", "errorMessage"] },
  "PAY-WEBHOOK-SIG-001":  { severity: "WARN",   retryable: false, userImpact: "none",     message: "Webhook aláírás/timestamp érvénytelen — elutasítva", requiredContext: ["provider"] },
  "PAY-WEBHOOK-VERIFY-001": { severity: "ERROR", retryable: true, userImpact: "degraded", message: "Webhook Retrieve Transaction verifikáció eltérés", requiredContext: ["provider", "transactionId"] },
  "PAY-WEBHOOK-AMOUNT-001": { severity: "CRITICAL", retryable: false, userImpact: "blocked", message: "Webhook összeg ≠ rendelés összeg", requiredContext: ["orderId", "orderTotal", "webhookAmount"] },
  "PAY-WEBHOOK-DUP-001":  { severity: "INFO",   retryable: false, userImpact: "none",     message: "Duplikált webhook event (idempotencia no-op)", requiredContext: ["provider", "externalId"] },

  // -- Számlázás (Billingo / NAV Online Számla / Xero) ----------------------
  "INV-CREATE-001":     { severity: "ERROR",    retryable: true,  userImpact: "degraded", message: "Számla létrehozás sikertelen (accounting provider)", requiredContext: ["provider", "orderId", "errorMessage"] },
  "INV-NAV-001":        { severity: "ERROR",    retryable: true,  userImpact: "none",     message: "NAV Online Számla riport sikertelen", requiredContext: ["orderId", "errorMessage"] },

  // -- Szállítás (Fáma Futár stb.) ------------------------------------------
  "SHIP-LABEL-001":     { severity: "ERROR",    retryable: true,  userImpact: "degraded", message: "Szállítási címke generálás sikertelen", requiredContext: ["carrier", "orderId"] },
  "SHIP-TRACK-001":     { severity: "WARN",     retryable: true,  userImpact: "none",     message: "Csomagkövetés frissítés sikertelen", requiredContext: ["carrier", "trackingNumber"] },

  // -- Keresés (Typesense, ADR-0002) ----------------------------------------
  "SEARCH-QUERY-001":   { severity: "WARN",     retryable: true,  userImpact: "degraded", message: "Typesense lekérdezés sikertelen (SQL fallback)", requiredContext: ["query"] },
  "SEARCH-SYNC-001":    { severity: "ERROR",    retryable: true,  userImpact: "none",     message: "Typesense sync (cron upsert) sikertelen", requiredContext: ["errorMessage"] },

  // -- Auth (Supabase Auth — customer; Cloudflare Access — admin) -----------
  "AUTH-SESSION-001":   { severity: "WARN",     retryable: false, userImpact: "blocked",  message: "Customer session ellenőrzés sikertelen", requiredContext: ["source"] },

  // -- Kapcsolati űrlap (contact form → email provider) ---------------------
  "CONTACT-SEND-001":   { severity: "ERROR",    retryable: true,  userImpact: "degraded", message: "Kapcsolati üzenet küldése sikertelen (email provider)", requiredContext: ["source"] },
  "CONTACT-SEND-002":   { severity: "WARN",     retryable: false, userImpact: "degraded", message: "Kapcsolati üzenet — nincs email provider konfigurálva (üzenet nem ment ki)", requiredContext: ["source"] },

  // -- Analitika (first-party, cookieless pageview beacon) ------------------
  "ANALYTICS-TRACK-001": { severity: "WARN",    retryable: false, userImpact: "none",     message: "Pageview beacon rögzítése sikertelen (analitika írás)", requiredContext: ["source"] },
...
// Module-load sanity: collect any PROJECT_CODES key that shadows a global code.
// SECURITY: NO `console.*` here (see file header) — the original pipeline logged
// this to `wrangler tail`, which we forbid. Instead we expose the conflicts as
// an exported array; `codes.test.ts` asserts it is empty, so a shadowing typo
// fails CI without ever touching the console or bricking the worker at runtime.
export const CODE_SHADOW_CONFLICTS: string[] = [];
for (const projectKey of Object.keys(PROJECT_CODES)) {
  for (const map of GLOBAL_MAPS) {
    if (Object.prototype.hasOwnProperty.call(map, projectKey)) {
      CODE_SHADOW_CONFLICTS.push(projectKey);
    }
  }
}
...
/** Engine-defined codes — a precise union (literal keys from PROJECT_CODES). */
export type EngineErrorCode = keyof typeof PROJECT_CODES;

/**
 * Any catalogue code. Engine call-sites should pass an `EngineErrorCode` (fully
 * type-checked); the global string codes (HTTP/SRV/…) are accepted too so a
 * route can log e.g. "HTTP-429-001" without a cast.
 */
export type ErrorCode = EngineErrorCode | (string & {});

/** Fallback used when an unknown code is logged — never throws. */
export const UNKNOWN_CODE_DEF: ErrorCodeDef = {
  severity: "ERROR",
  retryable: false,
  userImpact: "degraded",
  message: "Unknown error code (not in catalogue)",
  requiredContext: [],
};

/** Resolve a code's definition; unknown codes get `UNKNOWN_CODE_DEF`. */
export function getErrorCodeDef(code: string): ErrorCodeDef {
  return ALL_CODES[code] ?? UNKNOWN_CODE_DEF;
}
